...
# 9.3.2
# 这道题动态规划很简单。就是假设列表中第一个数为最大subsum，然后逐个将列表后的元素叠加到最大子序和中。如果最大子序和变大了，就修改最大子序和的value，继续往下叠加。如果变小了就放弃最后叠加的数字（必定是负数）。从最后叠加的那个数（负数）之后，重新开始叠加计算最大子序和。最后得到的就是整个列表的最大子序和。以nums=【-2，1，-3，4，-1，2，1，-5，4】为例。
